...
GROUP BY ?nature_id ?nature_frlabel
ORDER BY DESC(?count)' > wikidata_cote_d_ivoire_count_p31.csv
...
GROUP BY ?nature_id ?nature_frlabel
ORDER BY DESC(?count)' > wikidata_haiti_count_p31.csv
...
GROUP BY ?nature_id ?nature_frlabel
ORDER BY DESC(?count)' > wikidata_maurice_count_p31.csv
...
GROUP BY ?nature_id ?nature_frlabel
ORDER BY DESC(?count)' > wikidata_mali_count_p31.csv
...
GROUP BY ?nature_id ?nature_frlabel
ORDER BY DESC(?count)' > wikidata_senegal_count_p31.csv
